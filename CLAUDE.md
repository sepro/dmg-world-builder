...
**Theory tables** (top of script): `SCALES`, `PROGRESSIONS` (diatonic degree lists), `PATTERNS` (rhythmic archetypes), `MOODS` (parameter profiles), `DUTIES`/`WAVES`/`DRUMS`. Time is quantized to 16th-note steps (`STEPS_PER_QUARTER = 4`); `stepsPerBar = beats * (16/unit)`. Swing is baked into each note’s float onset `t`, and density changes note frequency while a shared articulation creates explicit silence between slots, so playback, the score, and MIDI all agree.
...
### Sound tools (`src/lib/gbsfx-*.js`, `src/legacy/gb-sfx-generator.js`, `src/legacy/gb-sfx-sequencer.js`)

Two pages over **one** model and **one** compiler: `gb-sfx-generator.html` designs single-tone effects (a hit, a laser, a coin), `gb-sfx-sequencer.html` authors chimes (a run of notes: victory, fail, UI). Both hold **one sound at a time** — exports are one sound per file, so a session library only made "which sound is this?" ambiguous — and both have JSON-snapshot undo instead.

The shared halves are:

- `src/lib/gbsfx-core.js` — DOM-free: the project/effect/layer/note model, the compile pipeline, the randomizer, the C exporter and the JSON import. Unit-tested (`gbsfx-core.test.js`).
- `src/lib/gbsfx-audio.js` — the Web Audio approximation of the four channels (`audio.scheduleLayer` serves both live preview and the offline WAV render).
- `src/lib/gbsfx-chimes.js` — procedural chime generation (sequencer only). Unit-tested (`gbsfx-chimes.test.js`).
- `src/legacy/gbsfx-ui.js` — the DOM pieces both engines share: undo history, the pitch/volume visualization, slider rows, the register inspector, and the import/export modals.
- `src/lib/gbsfx-glossary.js` + `src/legacy/gbsfx-glossary-ui.js` — the in-tool glossary (see below). Unit-tested (`gbsfx-glossary.test.js`).

The saved file is `.gbsfx.json` (`formatVersion 1`): a `name`, a `nextId` and an `effects` array (one entry in practice), each effect with a `tickHz` (default 60) and a list of `layers`. A layer targets one `channel` (`pulse1`/`pulse2`/`wave`/`noise`) and is `mode: "macro"` (generated), `mode: "manual"` (hand-edited `steps`), or `mode: "sequence"` (a list of `notes`). **Each tool edits only its own kind of layer and shows the other's read-only with a link across** (`crossToolNotice`) — opening a file in the "wrong" tool never drops anything, which is what lets a chime keep a hand-tuned noise underlay.
...
**Compile pipeline is the contract**: `compileLayer(effect, layer)` turns a macro (or manual steps, or a note sequence) into a per-frame program `{ vol 0..15, freqHz/note, noiseTone, duty, width, trigger, remain, env, silent }` plus `peakVol`/`decayPace`. Everything downstream consumes that program: the two-lane pitch/volume canvas (`drawViz`), the shared Web Audio scheduler (`audio.scheduleLayer`), and the C export. `trigger`/`remain`/`env`/`silent` are what the exporter needs and the reason a sequence needs no separate export path: where the channel is keyed, how much of the note is still to come, the envelope's start volume, and whether the frame writes at all.

**Sequences** (`compileSequence`): a `sequence` layer's `notes` are `{ note|noiseTone, len (frames), tie, vol, rest }`, played through the layer's macro timbre and envelope. Every note re-triggers the channel; `tie` carries the previous envelope through (so a tied run is one swell whose pitch steps) and `rest` writes nothing at all. Notes hold a steady pitch — bend/jump/vibrato are single-tone macros and are ignored, which is also why the sequence card offers only Punch/Decay/Sustain/Tone. Authoring is a piano roll and a note table side by side (`sequencePanel`/`drawRoll`/`seqTable`), editing one model: a value edit redraws the roll *and* the compiled visualization (a volume edit changes both — it used to change neither), a structural edit rebuilds the table, and a roll drag pushes values back into the inputs rather than rebuilding (which would drop focus mid-typing). The roll's bottom strip is the per-note volume lane, draggable like the pitches.

**Chime archetypes** (`gbsfx-chimes.js`): a chime is a shape, not a phrase. An archetype is a chord set, a contour (`up`/`down`/`updown`/`alt`), a note count, a step, a pacing shape (`even`/`accel`/`ritard`), a tail multiplier for the landing note, and a timbre range; `generateChime(key, seed)` is a pure deterministic draw from it (`MAX_CHIME_SPAN` keeps the walk inside two octaves). The layer keeps a `chime: { archetype, seed }` record so Re-roll and the seed box can draw again — an extra field the converter and the game ignore, and hand-editing the notes simply leaves it behind. `regenerateEffect` still never touches a sequence's notes.
...
**C export** (`exportC` → `gbsfx.h`/`gbsfx.c`): `buildEffectProgram`/`layerToRegisters` emit a compact byte program per effect for a tiny frame-stepped VM — `0x01 ch r0..r4` (write a channel's registers), `0x02 <16 bytes>` (load wave RAM once), `0x03 n` (end frame, then idle n more), `0xFF` (end frame), `0x00` (end effect). A channel is keyed with the hardware volume envelope + length counter set from the macro, so decays run on real hardware; frames inside a note rewrite only pitch (no re-trigger), avoiding the 60 Hz buzz that per-frame volume writes would cause on a DMG. **Every write loads the length counter with the time remaining**, not just the first (a write to NRx1 loads the counter directly, no trigger needed — writing zeros there instead resets it to maximum and the channel rings on ~250 ms past the program). That is also what makes compression safe: `significantRow` masks the length bits out of the change comparison, a frame that changes nothing else is skipped entirely (its counter keeps ticking to the same end point), and the resulting runs of empty frames collapse into `0x03 n`. The runtime API is `gbsfx_init()` (once), `gbsfx_play(id)`, and `gbsfx_update()` (once per frame, and from inside any long blocking loop or a sequence freezes mid-note).

**Glossary**: the tools are full of borrowed vocabulary (decay, duty, envelope, LFSR, tie), so every such word on screen is a dashed-underlined `term(key)` span — blurb on hover or keyboard focus, full entry in a modal on click, plus a browse-all `Glossary` button in the top bar. `src/lib/gbsfx-glossary.js` holds the entries (`short` / `plain` / `example` / `hardware`, in that order — plain English first, the register bridge last) and the A/B `demo` pairs, built as throwaway effects from one neutral base so a pair differs *only* in the term being explained; it stays DOM-free and audio-free so it can be unit-tested, and `src/legacy/gbsfx-glossary-ui.js` is the popover, the modal and the playback. Sliders and dropdowns get theirs from the label → key table `CONTROL_TERMS` in `gbsfx-ui.js`, so a call site never mentions the glossary; terms that cannot carry an underline (options inside a `<select>`, the register table's column headings) are folded into the entry of the control they belong to. `gbsfx-glossary.test.js` scans the two tool sources and fails on a key with no entry, an entry nothing points at, a control label missing from `CONTROL_TERMS`, and a demo that is silent, over-long, or identical to its partner. One rule the hover popover has to keep, and the reason it has an e2e case of its own: **both tools scroll inside `<main>`, not the window**, so a term can leave the screen without the window moving. The blurb therefore follows its term on every scroll (reaching a term below the fold by keyboard *is* a scroll — the browser brings it into view before the blurb is placed) but hides the moment that term is out of the viewport or clipped by a scrolling ancestor. Under the mouse `mouseleave` covers this; keyboard focus survives a scroll and does not.

The game repo's `tools/gbsfx2c.py` is a byte-for-byte port of this exporter (it is what actually builds the ROM's sound data); changing the compile or emit path here means changing it there in the same commit. Playback is a close Web Audio approximation, not a cycle-accurate emulator (e.g. duty is treated as constant across an effect in the preview).
...
The JSON file is the single source of truth. The C should never be hand-edited — regenerate it whenever the JSON changes. See `markdown/DEVELOPER_HANDOFF.md` for the full schema, generated C structures, and GBDK runtime integration snippets.
