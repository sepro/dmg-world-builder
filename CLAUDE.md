...
A bundled sound-effect designer for the four GB channels. The saved file is `.gbsfx.json` (`formatVersion 1`): a bank of `effects`, each with a `tickHz` (default 60) and a list of `layers`. A layer targets one `channel` (`pulse1`/`pulse2`/`wave`/`noise`) and is `mode: "macro"` (generated), `mode: "manual"` (hand-edited `steps`), or `mode: "sequence"` (a list of `notes`).
...
**Compile pipeline is the contract**: `compileLayer(effect, layer)` turns a macro (or manual steps, or a note sequence) into a per-frame program `{ vol 0..15, freqHz/note, noiseTone, duty, width, trigger, remain, env, silent }` plus `peakVol`/`decayPace`. Everything downstream consumes that program: the two-lane pitch/volume canvas (`drawViz`), the shared Web Audio scheduler (`audio.scheduleLayer`, used by both live preview and the offline WAV render via `renderWav`/`encodeWav`), and the C export. `trigger`/`remain`/`env`/`silent` are what the exporter needs and the reason a sequence needs no separate export path: where the channel is keyed, how much of the note is still to come, the envelope's start volume, and whether the frame writes at all.

**Sequences** (`compileSequence`, `SEQUENCE_PRESETS`): a `sequence` layer's `notes` are `{ note|noiseTone, len (frames), tie, vol, rest }`, played through the layer's macro timbre and envelope. Every note re-triggers the channel; `tie` carries the previous envelope through (so a tied run is one swell whose pitch steps) and `rest` writes nothing at all. Notes hold a steady pitch — bend/jump/vibrato are single-tone macros and are ignored, which is also why the sequence layer card hides those sliders. Authoring is a piano roll and a note table side by side (`sequencePanel`/`drawRoll`/`seqTable`), editing one model: a value edit redraws the roll, a structural edit rebuilds the table, and a roll drag pushes values back into the inputs rather than rebuilding (which would drop focus mid-typing). `regenerateEffect`/Mutate deliberately skip sequence layers' notes.
